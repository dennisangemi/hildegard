...
def extract_text_from_pdf_v0(pdf_path):
    ...
    for page_number in range(len(document)):
        print(f"Elaborazione pagina {page_number + 1}...")
...
def extract_text_from_pdf_v1(pdf_path):
    print("Estrazione testo da PDF...")
    # Apri il documento PDF
    document = fitz.open(pdf_path)
    data = []
...
def extract_text_from_pdf_v2(pdf_path):
    ...
    for page_number in range(len(document)):
        ...
        # Estrai l'immagine della pagina ritagliata
        pix = page.get_pixmap(clip=cropped_rect)
        img = Image.open(io.BytesIO(pix.tobytes()))
        
        # Usa Tesseract per estrarre il testo dall'immagine ritagliata
...
input_pdf = "liturgie/lezionari/pdf/lezionario_domenicale_festivo_anno_b.pdf"
output_csv = "liturgie/lezionari/raw_lezionario_anno_b_tesseract.csv"

# Estrai il testo dal PDF e salva nel CSV
text_data = extract_text_from_pdf_v2(input_pdf)
